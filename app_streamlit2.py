...
from routing_logic1 import COORDENADAS_LOTES, solve_route_optimization, VEHICLES, COORDENADAS_ORIGEN 
...
# Ocultar menú de Streamlit y footer para un look limpio
st.markdown("""
    <style>
...
if page == "Calcular Nueva Ruta":
    ...
    # --- INPUT Y VALIDACIÓN ---
    tab1, tab2 = st.tabs(["1. Entrada y Lotes", "2. Reporte de Resultados"])
    ...
    with tab1:
        ...
        # Pre-visualización de Lotes Seleccionados y Mapa
        map_data_list = []
            ...
            with st.spinner('Realizando cálculo óptimo y agrupando rutas (¡75s de espera incluidos!)...'):
                try:
                    ...
                    if "error" in results:
                        ...
                        st.rerun() # Fuerza el cambio de pestaña
                except Exception as e:
                    st.error(f"❌ Ocurrió un error inesperado durante el ruteo: {e}")
                    
    # Contenido de la Pestaña 2 (Reporte de Resultados)
    with tab2:
        if 'results' in st.session_state and st.session_state.results:
            ...
            st.header("Resumen General de la Optimización")
            ...
            st.warning("Aún no se ha realizado el cálculo. Ingrese los lotes en la pestaña anterior y presione 'Calcular'.")
